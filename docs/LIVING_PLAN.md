...
- Added a richer always-visible Status panel with phase/progress/elapsed/artifact details plus an opportunistic Live Snapshot area that extracts occasional stills only from completed/readable render artifacts.
...
- Added embedded final-delivery MP4 metadata for Plex-oriented fields (`title`, `show`, `season_number`, `episode_sort`, `episode_id`, `date`, `creation_time`, `description`, `synopsis`, `comment`, `genre`) and later switched those final MP4 exports to Plex-friendlier standard MP4/iTunes-style atoms instead of `+use_metadata_tags`.
...
- 2026-03-30: Replaced the hidden HDR `libx265` duration-based thread-profile auto-pick with an explicit export control: `Slow` (`4` pools / `2` frame threads), `Medium` (`5` pools / `2` frame threads, default), and `Fast` (`6` pools / `3` frame threads).
...
- 2026-03-20: Bumped the release line to `1.1.0`, rebuilt the packaged app as `1.1.0 (202)`, regenerated release status docs, and designated this merged randomized-collage build as durable rollback anchor `known-good/20260320-v1-1-0-collage-titles`.
