...
- 2026-03-10: Hotfix: PhotoKit direct-video render URLs are now cached together with the originating `AVURLAsset`, so `.photoslibrary/originals/...` inputs remain backed by a live PhotoKit object instead of becoming missing-file FFmpeg inputs after the URL object is dropped.
...
- 2026-03-10: Fixed PhotoKit direct-video retention by caching successful direct URLs together with their `AVURLAsset` owners, preventing `.photoslibrary/originals/...` paths from going stale before FFmpeg opens them while still avoiding broad temp-copy fallback.
